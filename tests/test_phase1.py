...
def test_findings_cap_applies_per_record() -> None:
    records = [
        SourceRecord(
            "First source",
            "https://example.test/first",
            "Alpha systems provide durable workflow state for agents. Beta checkpoints preserve progress across interrupted research sessions. Gamma tools validate bounded inputs before external execution. Delta events expose useful runtime progress to operators.",
            "web",
        ),
        SourceRecord(
            "Second source",
            "https://example.test/second",
            "Epsilon registries expose capabilities without implementation details. Zeta specialists own narrow tools and procedural skills. Eta orchestration delegates independent work across isolated contexts. Theta synthesis grounds factual claims in collected evidence.",
            "web",
        ),
    ]
    assert len(ResearchRuntime._findings(records, question="Research")) == 6


def test_single_authoritative_source_prefers_official_docs() -> None:
    records = [
        SourceRecord("Tutorial", "https://example.test/tutorial", "Community tutorial content.", "web"),
        SourceRecord("Deep Agents overview - Docs", "https://docs.langchain.com/deepagents", "Official documentation content.", "web"),
        SourceRecord("Repository", "https://github.com/example/project", "Repository content.", "web"),
    ]
    selected = ResearchRuntime._select_records(records, "Use one authoritative source.")
    assert [record.url for record in selected] == ["https://docs.langchain.com/deepagents"]
...
def test_run_writes_detailed_log(tmp_path) -> None:
    ...
    for marker in ("session_start", "decision", "agent_spawn", "subtask_started", "tool_call", "tool_result", "agent_result", "final_answer", "session_end"):
    ...
    assert all("result" not in entry for entry in entries if entry["kind"] == "agent_result")
